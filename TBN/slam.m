function slam()
    function draw(k,xhat,Gx,colortraj)
        if (mod(k,100)==0)
            draw_ellipse(xhat(1:2),Gx(1:2,1:2),0.9,colortraj,0.3);
            for i=4:2:length(xhat)-1,
                 draw_ellipse(xhat(i:i+1),Gx(i:i+1,i:i+1),0.9,'red',2);
            end
    ...
    function [y,C,Gbeta]=g(k),
        y=depth(k);
        C=zeros(1,nx);C(1,3)=1;Gbeta=0.01;
...
%%
   function filter()
        xhat=zeros(nx,1);
        Ghat=diag([0,0,0,10^6*ones(1,2*nm)]);
...
%%   
    function smoother()
        ...
        for k=1:kmax,
            u{k}=[dt*eulermat(phi(k),theta(k),psi(k))*vr(k,:)';zeros(2*nm,1)];
            [y,C,Gbeta]=g(k);
            [x_forw{k+1},G_forw{k+1},xup{k},Gup{k}]=kalman(x_forw{k},G_forw{k},u{k},y,Galpha,Gbeta,A,C);
            draw(k,x_forw{k},G_forw{k},'blue');
        ...
        for k=kmax-1:-1:1,
            ...
            draw(k,x_back{k},G_back{k},'black');
        end
